...
class CSV:
    FORMAT = "%d-%m-%Y"
    FOLDER = "C:/Users/drket/OneDrive/Desktop/Codes/PROJECTS/FINANCETRACKERPYTHON"
    CSV_FILE = os.path.join(FOLDER, "finance_data.csv")
    COLUMNS = ["date", "amount", "category", "description"]

    @classmethod  # it will have access to the class itself but wont have access to the instance of the class
    def initialize_csv(cls):
        try:
            pd.read_csv(cls.CSV_FILE)
        except FileNotFoundError:
            df = pd.DataFrame(columns=cls.COLUMNS)
            # DATAFRAME = object within pandas that allows us to different rows/columns from a csv file
    ...
    def get_transaction(cls, start_date, end_date):
        ...
        # & = bitwise and operator used when working with pandas dataframe or mask specifically
...
def plot_transactions(df):
    ...
    # resample fills all the missing days with 0
    income_df = (
...
def main():
    while True:
        ...
        print("2. Get transaction history")
        print("3. Exit")
